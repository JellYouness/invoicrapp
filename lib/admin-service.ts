...
class AdminService {
  ...
  async getAdminDashboardStats(
    ...
    try {
      ...
      if (analyticsError) {
        console.error("Error fetching analytics history:", analyticsError);
        return null;
      }
      ...
      topUsersData?.forEach((invoice) => {
        ...
        userRevenueMap.set(invoice.user_id, {
          totalRevenue: existing.totalRevenue + Number(invoice.total_amount),
      ...
      const topUsersWithEmails = sortedUsers.map(([userId, stats]) => {
        const profile = topUsersProfiles?.find((p) => p.user_id === userId);
      ...
      const subscriptionBreakdown = subscriptions
        ? [
            {
              plan: "Free",
              count: subscriptions.filter((s) => s.plan_type === "free").length,
              percentage: 0,
            },
            {
              plan: "Pro",
              count: subscriptions.filter((s) => s.plan_type === "pro").length,
              percentage: 0,
            },
          ]
        : [];
    ...
    try {
      ...
      invoiceStats?.forEach((invoice) => {
        ...
        userStatsMap.set(invoice.user_id, {
          ...
          revenue:
            existing.revenue +
            (invoice.status === "paid" ? Number(invoice.total_amount) : 0),
        });
      });
