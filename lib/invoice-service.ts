...
export const updateInvoice = async (id: string, updates: Partial<CreateInvoiceData>): Promise<SavedInvoice | null> => {
  try {
    const { data: { user } } = await supabase.auth.getUser();
...
export const getInvoicesByClient = async (clientName: string): Promise<SavedInvoice[]> => {
  try {
    const { data: { user } } = await supabase.auth.getUser();
...
export const getInvoiceCountByClient = async (clientName: string): Promise<number> => {
  try {
    const { data: { user } } = await supabase.auth.getUser();
...
export const getInvoiceCountsForClients = async (clientNames: string[]): Promise<Record<string, number>> => {
  try {
    const { data: { user } } = await supabase.auth.getUser();
...
export interface InvoiceAnalytics {
  ...
  topClients: Array<{ name: string; totalRevenue: number; invoiceCount: number }>;
  recentActivity: Array<{ date: string; action: string; invoice: string; amount: number }>;
}

// Get comprehensive analytics data
export const getInvoiceAnalytics = async (period: '30days' | '6months' | '1year' = '6months'): Promise<InvoiceAnalytics | null> => {
  try {
    const { data: { user } } = await supabase.auth.getUser();
    ...
    switch (period) {
      case '30days':
        ...
        break;
      case '6months':
        dateFilter = new Date(now.getTime() - 6 * 30 * 24 * 60 * 60 * 1000).toISOString();
        break;
      case '1year':
    ...
    const totalRevenue = invoices.reduce((sum: number, inv: SavedInvoice) => 
      inv.status === 'paid' ? sum + inv.total_amount : sum, 0);
    
    const paidInvoices = invoices.filter((inv: SavedInvoice) => inv.status === 'paid').length;
    const pendingInvoices = invoices.filter((inv: SavedInvoice) => 
      inv.status === 'draft' || inv.status === 'sent').length;
    const overdueInvoices = invoices.filter((inv: SavedInvoice) => inv.status === 'overdue').length;
    
    const averageInvoiceValue = totalInvoices > 0 ? 
      invoices.reduce((sum: number, inv: SavedInvoice) => sum + inv.total_amount, 0) / totalInvoices : 0;
    ...
    // Top clients
    const clientData: Record<string, { totalRevenue: number; invoiceCount: number }> = {};
    invoices.forEach((inv: SavedInvoice) => {
      if (!clientData[inv.client_name]) {
        clientData[inv.client_name] = { totalRevenue: 0, invoiceCount: 0 };
      }
      if (inv.status === 'paid') {
